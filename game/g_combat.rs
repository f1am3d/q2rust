...
pub unsafe extern "C" fn M_ReactToDamage(
    ...
    if !((*attacker).client).is_null() {
        (*targ).monsterinfo.aiflags &= !(0x4 as libc::c_int);
    ...
    } else if !((*attacker).enemy).is_null() && (*attacker).enemy != targ {
        if !((*targ).enemy).is_null() && !((*(*targ).enemy).client).is_null() {
            let ref mut fresh11 = (*targ).oldenemy;
            *fresh11 = (*targ).enemy;
        }
        let ref mut fresh12 = (*targ).enemy;
        *fresh12 = (*attacker).enemy;
        if (*targ).monsterinfo.aiflags & 0x800 as libc::c_int == 0 {
            FoundTarget(targ);
        }
    }
